 ...
 *
 * ВАЖНО: клиентским компонентам импортировать константы только напрямую
 * по пути файла (`@/shared/lib/auth/constants`), не через баррель
 * `@/shared/lib/auth` — баррель тащит серверные модули (`next/headers`,
 * `jsonwebtoken`, `server-only`) и ломает клиентскую сборку.
 */
...
/**
 * Лимит попыток применения токена сброса с одного IP за окно.
 * Щедрый: токен 256-бит, перебор бессмысленен — лимит только от флуда
 * и не должен блокировать пользователя, пару раз опечатавшегося в пароле.
 */
export const RATE_LIMIT_RESET_PASSWORD = 10

/** Окно (в миллисекундах) для per-IP лимита применения токена сброса. */
