...
Email + пароль. Регистрация и вход — через server actions. Сессия — JWT (7 дней) в httpOnly-cookie; logout отзывает все сессии серверно (счётчик `token_version`). Защита от подбора: per-IP блокировка — после 5 неудачных попыток с одного IP (`ip:email`) этот IP блокируется на 15 минут; аккаунт при этом не блокируется, и попытки чужих IP не влияют на вход владельца. Восстановление пароля — одноразовая ссылка на email (токен хранится в БД только в виде SHA-256, срок действия 30 минут); после смены пароля выполняется автовход и отзыв прежних сессий.
