...
Email + пароль. Регистрация и вход — через server actions. Сессия — JWT (7 дней) в httpOnly-cookie; logout отзывает все сессии серверно (счётчик `token_version`). Защита от подбора: блокировка аккаунта после 5 неудачных попыток на 15 минут. Восстановление пароля — одноразовая ссылка на email (токен хранится в БД только в виде SHA-256, срок действия 30 минут); после смены пароля выполняется автовход и отзыв прежних сессий.
